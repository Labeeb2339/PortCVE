...
public sealed class CliApplication
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            output.WriteLine($"BindWitness {Version}");
    ...
    {
        output.WriteLine("BindWitness explains local ports and locks the ones you expect.");
        output.WriteLine();
        output.WriteLine("USAGE");
        output.WriteLine("  bindwitness                         List local TCP listeners and UDP binds");
        output.WriteLine("  bindwitness 8080                    Explain every local endpoint on port 8080");
        output.WriteLine("  bindwitness tcp:8080 --evidence     Explain one protocol with firewall evidence");
        output.WriteLine("  bindwitness lock -o listeners.lock  Save a normalized, privacy-reduced baseline");
        output.WriteLine("  bindwitness diff listeners.lock     Show drift from the live machine");
        output.WriteLine("  bindwitness check listeners.lock    Fail on new, wider, or owner-changed binds");
        output.WriteLine("  bindwitness watch --json             Stream endpoint changes as JSONL");
        output.WriteLine("  bindwitness doctor                  Check collection coverage and privacy mode");
        output.WriteLine();
        ...
        output.WriteLine();
        output.WriteLine("BindWitness is read-only and does not prove Internet reachability.");
        return ExitCodes.Success;
    }
